...
let ReservationController = class ReservationController {
    ...
    async create(body) {
        ...
        if (result.error === 'duplicate_reservation') {
            throw new common_1.HttpException(result, common_1.HttpStatus.CONFLICT);
        }
        return result;
    }
    async getOne(reservationId) {
        const r = await this.reservationService.findByReservationId(reservationId);
        if (!r)
            throw new common_1.HttpException({ error: 'not_found' }, common_1.HttpStatus.NOT_FOUND);
        return r;
    }
};
__decorate([
...
__decorate([
    ...
    __metadata("design:returntype", Promise)
], ReservationController.prototype, "getOne", null);
